...
public class Dark_blue extends AppCompatActivity implements View.OnClickListener, SensorEventListener {
    ...
    private final BroadcastReceiver stop_fake_call = new BroadcastReceiver() { // from class: com.softdroid.fake.call.Call_Screens.Dark_blue.1
        @Override // android.content.BroadcastReceiver
        public void onReceive(Context context, Intent intent) {
            Dark_blue.this.stopPlaying();
            try {
                Dark_blue.this.r.stop();
            } catch ( Exception unused) {
            }
            try {
                if (Dark_blue.this.sharedPreferences.getBoolean(Dark_blue.this.getResources().getString(R.string.vibration_pref), true)) {
                    Dark_blue.this.vibrator.cancel();
                }
            } catch ( Exception unused2) {
            }
            try {
                Dark_blue.this.cutom_ringtone_player.stop();
            } catch ( Exception unused3) {
            }
            Dark_blue.this.finish();
        }
    };
    ...
    public void onCreate(Bundle bundle) {
        ...
        setContentView(R.layout.activity_dark_blue);
    ...
    private void initilize_componenets() {
        ...
        this.ripple = (LottieAnimationView) findViewById(R.id.progressBar);
        this.main_layout = (RelativeLayout) findViewById(R.id.main_layout);
        this.ripple.setVisibility(View.VISIBLE);
        this.pick_call.setOnTouchListener(new View.OnTouchListener() { // from class: com.softdroid.fake.call.Call_Screens.Dark_blue.2
            @Override // android.view.View.OnTouchListener
            public boolean onTouch(View view, MotionEvent motionEvent) {
                if (motionEvent.getAction() == 2) {
                    Dark_blue.this.caling_ui.setVisibility(View.GONE);
                    Dark_blue.this.cancel_call.setVisibility(View.GONE);
                    Dark_blue.this.reject_call_layout.setVisibility(View.GONE);
                    Dark_blue.this.timer_textview.setVisibility(View.GONE);
                    Dark_blue.this.accpet_call_layout.setVisibility(View.GONE);
                    Dark_blue.this.cancel_call_new.setVisibility(View.VISIBLE);
                    Dark_blue.this.action_layout.setVisibility(View.VISIBLE);
                    if (Dark_blue.this.sharedPreferences.getBoolean(Dark_blue.this.getResources().getString(R.string.vibration_pref), true)) {
                        Dark_blue.this.vibrator.cancel();
                    }
                    try {
                    ...
                    try {
                        Dark_blue.this.mMediaPlayer_default.stop();
                    } catch ( Exception unused2) {
                    }
                    try {
                        Dark_blue.this.r.stop();
                    ...
                    try {
                        if (new File(Dark_blue.this.mFileName).exists()) {
                            Dark_blue.this.startPlaying();
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                    Dark_blue.this.fake_call_picked = true;
                    Dark_blue.this.chronometer.setBase(SystemClock.elapsedRealtime());
                    Dark_blue.this.chronometer.setVisibility(View.VISIBLE);
                    Dark_blue.this.chronometer.stop();
                    Dark_blue.this.chronometer.start();
                    Dark_blue.this.dial_active = true;
                }
                return true;
            }
        });
    ...
    public void onClick(View view) {
        ...
        switch (view.getId()) {
            ...
            case R.id.cancel_call /* 2131230854 */:
                ...
                this.reject_call_layout.setVisibility(View.GONE);
                this.accpet_call_layout.setVisibility(View.GONE);
                if (this.sharedPreferences.getBoolean(getResources().getString(R.string.vibration_pref), true)) {
                ...
                this.btoom_call_layout.setVisibility(View.VISIBLE);
    ...
    private void finish_Timer() {
        new Handler().postDelayed(new Runnable() { // from class: com.softdroid.fake.call.Call_Screens.Dark_blue.3
            @Override // java.lang.Runnable
            public void run() {
                try {
                    Dark_blue.this.mSensorManager.unregisterListener(Dark_blue.this);
                } catch ( Exception unused) {
                }
                Dark_blue.this.sharedPreferences.edit().putBoolean("fake_call_actvie", false).apply();
                Dark_blue.this.finish();
            }
        }, 1600);
    }
    ...
    private void finish_call_when_not_picked_up() {
        new Handler().postDelayed(new Runnable() { // from class: com.softdroid.fake.call.Call_Screens.Dark_blue.4
            @Override // java.lang.Runnable
            public void run() {
                if (!Dark_blue.this.fake_call_picked.booleanValue()) {
                    Dark_blue.this.sharedPreferences.edit().putBoolean("fake_call_actvie", false).apply();
                    if (Dark_blue.this.sharedPreferences.getBoolean(Dark_blue.this.getResources().getString(R.string.vibration_pref), true)) {
                        Dark_blue.this.vibrator.cancel();
                    }
